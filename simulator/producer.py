...
KAFKA_BOOTSTRAP_SERVERS = os.getenv("KAFKA_BOOTSTRAP_SERVERS")      # Port Number that we need to connect to Kafka
KAFKA_TOPIC = os.getenv("KAFKA_TOPIC", "spotify-events")          # The topic where the data is streamed
USER_COUNT = int(os.getenv("USER_COUNT", 20))
EVENT_INTERVAL_SECONDS = int(os.getenv("EVENT_INTERVAL_SECONDS", 1))  # Time delay between each row generated
...
producer = KafkaProducer(
    ...
    value_serializer=lambda v: json.dumps(v).encode("utf-8") # Before streaming we need to convert the data into bytes
)
